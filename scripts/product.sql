...
WHERE new_parent_id IN (
    ...
    FROM (
        SELECT new_parent_id 
        FROM catalog_product_bundle_option 
        GROUP BY new_parent_id 
        HAVING COUNT(*) > 1
    ) AS duplicates
);
...
WHERE option_id IN (
    SELECT option_id FROM (
        SELECT v.option_id
        FROM catalog_product_bundle_option_value v
        LEFT JOIN catalog_product_bundle_option o ON v.option_id = o.option_id
...
WHERE option_id IN (
    SELECT option_id FROM (
        ...
        LEFT JOIN catalog_product_bundle_option o ON s.option_id = o.option_id
        WHERE o.option_id IS NULL
    ) AS temp_table
);
...
ALTER TABLE catalog_product_bundle_selection DROP FOREIGN KEY CAT_PRD_BNDL_SELECTION_PRD_ID_SEQUENCE_PRD_SEQUENCE_VAL;
ALTER TABLE catalog_product_super_link DROP FOREIGN KEY CAT_PRD_SPR_LNK_PRD_ID_SEQUENCE_PRD_SEQUENCE_VAL;
